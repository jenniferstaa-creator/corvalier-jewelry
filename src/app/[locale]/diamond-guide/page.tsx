...
export async function generateMetadata({
  ...
  return {
    title: dict.diamondGuide.metaTitle,
    description: dict.diamondGuide.metaDescription,
  };
}
...
export default async function DiamondGuidePage({
  ...
  const locale = (isLocale(raw) ? raw : "en") as Locale;
  const dict = await getDictionary(locale);
  const { intro, sections, faqs } = dict.diamondGuide;

  return (
    <>
      <PageHero
        eyebrow={dict.diamondGuide.heroEyebrow}
        title={intro.title}
        intro={intro.lead}
      />
      ...
      <section className="bg-burgundy-deep text-ivory">
        <div className="maison-container py-24 md:py-32">
          <Reveal>
            <p className="eyebrow text-champagne">{dict.diamondGuide.faqEyebrow}</p>
            <h2 className="mt-4 font-display text-4xl">{dict.diamondGuide.faqTitle}</h2>
          </Reveal>
        ...
        <div className="maison-container py-24 text-center md:py-32">
          <Reveal>
            <h2 className="font-display text-4xl text-espresso md:text-5xl">
              {dict.diamondGuide.closingTitle}
            </h2>
            <p className="mx-auto mt-6 max-w-lg font-serif text-xl leading-relaxed text-espresso/65">
              {dict.diamondGuide.closingBody}
            </p>
          </Reveal>
          <Reveal delay={150}>
            <LocaleLink href="/booking" className="btn-primary mt-12">
